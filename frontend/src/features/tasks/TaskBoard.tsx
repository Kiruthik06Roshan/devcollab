    ...
    <div className="space-y-6">
      <Card className="border-white/10 bg-white/5">
        <CardContent className="flex flex-col gap-4 p-6 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <Badge className="border-cyan-400/20 bg-cyan-400/10 text-cyan-200">Kanban board</Badge>
            <h1 className="mt-3 text-3xl font-semibold tracking-tight text-white">{board?.project.name}</h1>
...
function TaskColumn({
  ...
  return (
    <Card ref={setNodeRef} className={`min-h-[32rem] border-white/10 ${isOver ? 'bg-white/8' : 'bg-white/5'}`}>
...
function TaskCard({
  ...
  return (
    <div ref={setNodeRef} style={style} {...attributes} className="rounded-2xl border border-white/10 bg-slate-950/60 p-4 shadow-soft">
      <div className="flex items-start gap-3">
        <button {...listeners} className="mt-1 text-slate-500 hover:text-slate-300">
        ...
        <div className="min-w-0 flex-1 space-y-3">
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <div className="mt-1 text-xs text-slate-500">Due {formatDueDate(task.dueDate)}</div>
            </div>
            <Badge className={priorityStyles[task.priority]}>{task.priority}</Badge>
          </div>
          {task.description ? <p className="text-sm leading-6 text-slate-400">{task.description}</p> : null}
          {task.labels.length ? (
...
function TaskModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/60 px-4 py-8 backdrop-blur-sm">
      <Card className="w-full max-w-2xl border-white/10 bg-slate-950 shadow-soft">
        <CardHeader className="border-white/5">
          <div className="flex items-center justify-between gap-3">
            <div>
              <div className="text-sm text-slate-400">{projectName}</div>
              <h2 className="text-2xl font-semibold text-white">{editing ? 'Edit task' : 'Create task'}</h2>
            </div>
        ...
        <CardContent className="p-6">
          <form className="space-y-4" onSubmit={onSubmit}>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div className="space-y-2">
                <label className="text-sm text-slate-300">Column</label>
                <select
                  value={form.status}
